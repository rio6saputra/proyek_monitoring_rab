...
function getSavedEntries($koneksi, $conditions) { 
    // [PERUBAHAN] Menambahkan kolom revisi ke query
    $sql = "SELECT e.id_rab_entry, e.komponen_id, e.sub_komponen_id, e.sub_komponen_2_id, a.id_akun, a.kode_akun, a.uraian_akun, e.memiliki_revisi, e.status_anggaran
            ...
            WHERE "; 
    $where_clauses = []; $params = []; $types = ""; foreach($conditions as $key => $value) { if ($value === null) { $where_clauses[] = "e.{$key} IS NULL"; } else { $where_clauses[] = "e.{$key} = ?"; $params[] = $value; $types .= is_int($value) ? "i" : "s"; } } $sql .= implode(" AND ", $where_clauses); $stmt = mysqli_prepare($koneksi, $sql); if (!empty($params)) { mysqli_stmt_bind_param($stmt, $types, ...$params); } mysqli_stmt_execute($stmt); $result = mysqli_stmt_get_result($stmt); $entries = []; $entry_ids = []; while ($row = mysqli_fetch_assoc($result)) { $row['total_anggaran'] = 0; $entries[$row['id_rab_entry']] = $row; $entry_ids[] = $row['id_rab_entry']; } 
    if (!empty($entry_ids)) { 
        $placeholders = implode(',', array_fill(0, count($entry_ids), '?')); 
        // [PERUBAHAN] Mengambil semua kolom, termasuk kolom revisi baru
        $sql_details = "SELECT * FROM RAB_Detail WHERE rab_entry_id IN ($placeholders)"; 
        $stmt_details = mysqli_prepare($koneksi, $sql_details); mysqli_stmt_bind_param($stmt_details, str_repeat('i', count($entry_ids)), ...$entry_ids); mysqli_stmt_execute($stmt_details); $result_details = mysqli_stmt_get_result($stmt_details); $details_flat = []; $detail_ids = []; while($detail = mysqli_fetch_assoc($result_details)){ $details_flat[$detail['id_rab_detail']] = $detail; $detail_ids[] = $detail['id_rab_detail']; } 
        if (!empty($detail_ids)) { $placeholders_vol = implode(',', array_fill(0, count($detail_ids), '?')); $sql_volumes = "SELECT * FROM RAB_Volume WHERE rab_detail_id IN ($placeholders_vol)"; $stmt_volumes = mysqli_prepare($koneksi, $sql_volumes); mysqli_stmt_bind_param($stmt_volumes, str_repeat('i', count($detail_ids)), ...$detail_ids); mysqli_stmt_execute($stmt_volumes); $result_volumes = mysqli_stmt_get_result($stmt_volumes); while($volume = mysqli_fetch_assoc($result_volumes)){ $details_flat[$volume['rab_detail_id']]['volumes'][] = $volume; } } 
        $details_by_entry = []; 
        foreach($details_flat as $detail) { 
            $details_by_entry[$detail['rab_entry_id']][] = $detail; } 
        foreach($details_by_entry as $entry_id => $details) { 
            $entries[$entry_id]['details'] = build_detail_tree($details); 
            $total_entry = 0; 
        foreach($details_by_entry as $entry_id => $details) {
            $entries[$entry_id]['details'] = build_detail_tree($details);
            ...
            foreach ($entries[$entry_id]['details'] as $top_level_detail) {
                $total_entry += $top_level_detail['total_biaya'];
            }
...
function calculate_total(&$hierarchy) { $total = 0; if (isset($hierarchy['sub_komponens_2'])) { foreach ($hierarchy['sub_komponens_2'] as &$sk2) { $total += calculate_total($sk2); } } if (isset($hierarchy['sub_komponens'])) { foreach ($hierarchy['sub_komponens'] as &$sk) { $total += calculate_total($sk); } } if (isset($hierarchy['komponens'])) { foreach ($hierarchy['komponens'] as &$komp) { $total += calculate_total($komp); } } if (isset($hierarchy['saved_data'])) { foreach ($hierarchy['saved_data'] as $saved) { $total += $saved['total_anggaran']; } } $hierarchy['total_anggaran'] = $total; return $total; }
