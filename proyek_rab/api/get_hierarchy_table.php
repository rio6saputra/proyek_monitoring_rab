...
function calculate_total(&$hierarchy) {
    $total = 0;
    $total_revisi = 0; // Tambahkan variabel untuk total revisi

    // Rekursif untuk semua level di bawahnya
    if (isset($hierarchy['sub_komponens_2'])) { foreach ($hierarchy['sub_komponens_2'] as &$sk2) { $child_totals = calculate_total($sk2); $total += $child_totals['total']; $total_revisi += $child_totals['total_revisi']; } }
    if (isset($hierarchy['sub_komponens'])) { foreach ($hierarchy['sub_komponens'] as &$sk) { $child_totals = calculate_total($sk); $total += $child_totals['total']; $total_revisi += $child_totals['total_revisi']; } }
    if (isset($hierarchy['komponens'])) { foreach ($hierarchy['komponens'] as &$komp) { $child_totals = calculate_total($komp); $total += $child_totals['total']; $total_revisi += $child_totals['total_revisi']; } }
    ...
    $hierarchy['total_anggaran_revisi'] = $total_revisi; // Simpan total revisi
    
    return ['total' => $total, 'total_revisi' => $total_revisi]; // Kembalikan kedua nilai
}
