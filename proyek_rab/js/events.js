...
const parseAndValidateDetailForm = (formElement) => {
    ...
    const headerUraianInput = formElement.querySelector('#input-header-uraian');
    const detailRows = formElement.querySelectorAll('.detail-form-grid-row');
    const details = Array.from(detailRows).map(row => {
        const id_rab_detail = row.dataset.idDetail || null;
        const uraian = row.querySelector('.uraian-input').value.trim();
        const harga_satuan = row.querySelector('.harga-input').value.replace(/\./g, '');
        const volumes = [];
        for (let i = 0; i < 4; i++) {
            const volumeInput = row.querySelectorAll('.volume-input')[i];
            const satuanInput = row.querySelectorAll('.satuan-input')[i];
            if (volumeInput && satuanInput && volumeInput.value && satuanInput.value) {
                volumes.push({
    ...
    if (headerUraianInput && headerUraianInput.value.trim()) {
        return [{
            uraian: headerUraianInput.value.trim(),
            harga_satuan: null,
            volumes: [],
            children: details
        }];
...
const handleContextMenu = (e) => {
    if (elements.rabContainer.dataset.isLocked === 'true') {
        e.preventDefault();
        dom.showToast('info', 'Anggaran telah dikunci dan tidak bisa diubah.');
        return;
    }
...
export const initializeEventListeners = () => {
    ...
    if (elements.rabContainer) {
        elements.rabContainer.addEventListener('click', (e) => {
            ...
            if (target.classList.contains('toggle-btn')) {
                ...
                dom.openEditModal(context.id_rab_detail, context.rab_entry_id);
            } else if (target.classList.contains('status-revisi')) {
                if (window.isAdmin) openVerificationModalForAdmin(target.dataset.idDetail);
            }
        });
    ...
    if (elements.modal) {
        ...
        elements.modal.addEventListener('submit', (e) => {
            ...
            if (form.id === 'add-new-detail-form' || form.id === 'input-detail-form') {
                 ...
                 if (!details) return;
                 const payload = { rab_entry_id: form.dataset.entryId, details: details };
    ...
    if (elements.editModal) {
        elements.editModal.addEventListener('submit', (e) => {
            e.preventDefault();
            const form = e.target;
            if (form.id === 'edit-detail-form') {
                const details = parseAndValidateDetailForm(form);
                if (!details) return;
                const payload = { rab_entry_id: form.dataset.entryId, details: details };
                api.simpanBanyakDetail(payload).then(result => {
                    if (result.status === 'success') {
                        dom.showToast('success', result.message);
                        dom.closeEditModal();
                        const expandedPaths = dom.getExpandedPaths();
    ...
    if (elements.contextMenu) {
        elements.contextMenu.addEventListener('click', (e) => {
            ...
            if (action === 'hapus') {
                ...
                if (context.id_rab_detail) dom.openEditModal(context.id_rab_detail, context.rab_entry_id);
