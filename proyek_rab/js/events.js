        ...
        if (result.isConfirmed && apiCall) {
            apiCall(payload).then(res => {
                if (res.status === 'success') {
                    ...
                    // --- TAMBAHKAN BLOK KODE INI UNTUK REFRESH TAMPILAN ---
                    const expandedPaths = dom.getExpandedPaths();
                    dom.loadHierarchyTable('rab-container', state.selectedKegiatanId, state.selectedKroId, state.selectedBagianId, () => {
                        dom.restoreExpandedState(expandedPaths);
                    });
...
export const initializeEventListeners = () => {
    ...
    // Di dalam file: proyek_rab/js/events.js
...
if (elements.modal) {
    elements.modal.addEventListener('click', (e) => {
        ...
        if (target.id === 'btn-rekam-akun') {
            ...
            const payload = { 
                context: JSON.parse(e.target.dataset.context), 
                id_akun: select.value, 
                kode_akun: selectedOption.dataset.kodeakun,
            ...
            const rowToDelete = target.closest('.detail-form-grid-row');
            if (!rowToDelete) return;

            // --- LOGIKA BARU YANG DISEMPURNAKAN UNTUK UNLOCK ---
            if (rowToDelete.classList.contains('sub-detail-row')) {
                let parentRow = rowToDelete.previousElementSibling;
                // Cari parent row yang sebenarnya, lewati sub-detail lainnya
                while(parentRow && parentRow.classList.contains('sub-detail-row')) {
                    parentRow = parentRow.previousElementSibling;
                }

                // Hitung jumlah anak yang tersisa SETELAH baris ini dihapus
                let siblingCount = 0;
                let nextSibling = parentRow.nextElementSibling;
                while(nextSibling && nextSibling.classList.contains('sub-detail-row')) {
                    if (nextSibling !== rowToDelete) { // Jangan hitung baris yang akan dihapus
                        siblingCount++;
                    }
                    nextSibling = nextSibling.nextElementSibling;
                }
                
                // Jika tidak ada anak yang tersisa, buka kunci parent
                if (parentRow && siblingCount === 0) {
                    parentRow.querySelectorAll('input, textarea').forEach(input => {
                        input.readOnly = false;
                    });
                    parentRow.querySelectorAll('button').forEach(button => {
                        button.disabled = false;
                    });
                    parentRow.classList.remove('is-locked');
                }
            }
            // --- AKHIR LOGIKA BARU ---
            ...
            dom.calculateTotalBiaya(target.closest('form'));
                ...
                // --- LOGIKA BARU YANG DISEMPURNAKAN UNTUK LOCK ---
                // Kunci semua input teks, tapi biarkan tombol tetap aktif
                parentRow.querySelectorAll('input, textarea').forEach(input => {
                    input.readOnly = true;
                });
                // Tambahkan class untuk styling latar belakang
                parentRow.classList.add('is-locked');
                // --- AKHIR LOGIKA BARU ---
    ...
    elements.modal.addEventListener('submit', (e) => {
        ...
        if (form.id === 'add-new-detail-form' || form.id === 'input-detail-form') {
            ...
            api.simpanDetailBaru(payload).then(result => {
                    if (result.status === 'success') {
                        dom.showToast('success', result.message);
                        dom.closeAkunModal();
                        const expandedPaths = dom.getExpandedPaths();
