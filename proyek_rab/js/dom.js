...
export const createDetailInputRowHTML = (detail = {}, isSubDetail = false) => {
    // 1. Tambahkan kelas 'sub-detail-row' jika ini adalah rincian anak
    const rowClass = isSubDetail ? 'sub-detail-row' : '';
    const detailIdAttr = detail.id_rab_detail ? `data-id-detail="${detail.id_rab_detail}"` : '';

    const volumes = JSON.parse(detail.volume_data || '[]');
    let volumePairs = '';
    for (let i = 0; i < 5; i++) {
        const vol = volumes[i]?.volume || '';
        const sat = volumes[i]?.satuan || '';
        volumePairs += `
    ...
    // 2. Tambahkan tombol '+ Sub' jika ini BUKAN sub-detail
...
const createRowHtml = (level, path, name, itemData, parentContext, isReadOnly = false) => {
    ...
    const totalAnggaranRevisiFormatted = (itemData.total_anggaran_revisi && itemData.total_anggaran_revisi !== itemData.total_anggaran) 
        ? formatRupiah(itemData.total_anggaran_revisi) 
        : (state.currentStatus === 'REVISI_DRAFT' ? totalAnggaranFormatted : ''); // Tampilkan juga jika dalam mode revisi
    // --- PERBAIKAN SELESAI ---
    ...
    if (itemData.saved_data && itemData.saved_data.length > 0) {
        itemData.saved_data.forEach(entry => {
            ...
            const totalAnggaranAkunRevisiFormatted = (entry.total_anggaran_revisi && entry.total_anggaran_revisi !== entry.total_anggaran)
                ? formatRupiah(entry.total_anggaran_revisi)
                : (state.currentStatus === 'REVISI_DRAFT' ? totalAnggaranAkunFormatted : '');
...
export const loadHierarchyTable = (targetId, kegiatanId, kroId, bagianId, callback, isReadOnly = false) => {
    ...
    tableBodyHtml += createRowHtml('level-kro', kroMak, kroUraian, { total_anggaran: kroTotal, komponens: data, kro_id: kroMak }, {}, isReadOnly);
...
safeArray(data).forEach(ro => {
        ...
        tableBodyHtml += createRowHtml('level-ro', roPath, ro.nama_ro, ro, { path: kroMak, ro_id: ro.ro_id }, isReadOnly); // <-- Tambahkan isReadOnly
        safeArray(ro.komponens).forEach(komp => {
            const kompPath = `${roPath}.${komp.kode_komponen}`;
            tableBodyHtml += createRowHtml('level-komponen', kompPath, komp.nama_komponen, komp, { path: roPath, ro_id: ro.ro_id, komponen_id: komp.komponen_id }, isReadOnly); // <-- Tambahkan isReadOnly
            safeArray(komp.sub_komponens).forEach(sub => {
                const subPath = `${kompPath}.${sub.kode_sub_komponen}`;
                tableBodyHtml += createRowHtml('level-sub', subPath, sub.nama_sub_komponen, sub, { path: kompPath, ro_id: ro.ro_id, komponen_id: komp.komponen_id, sub_komponen_id: sub.sub_komponen_id }, isReadOnly); // <-- Tambahkan isReadOnly
                safeArray(sub.sub_komponens_2).forEach(sub2 => {
                    const sub2Path = `${subPath}.${sub2.kode_sub_komponen_2}`;
                    tableBodyHtml += createRowHtml('level-sub2', sub2Path, sub2.nama_sub_komponen_2, sub2, { path: subPath, ro_id: ro.ro_id, komponen_id: komp.komponen_id, sub_komponen_id: sub.sub_komponen_id, sub_komponen_2_id: sub2.sub_komponen_2_id }, isReadOnly); // <-- Tambahkan isReadOnly
                });
            });
        });
    });
...
export const openEditModal = (detailId, entryId, mode = 'edit') => {
    let detailToEdit = state.detailDataCache[detailId];
    if (!detailToEdit) {
        showToast('error', 'Data detail tidak ditemukan.');
        return;
    }
    if (detailToEdit.parent_detail_id) {
        // Jika item ini punya parent, maka yang sebenarnya mau kita edit adalah parent-nya.
        detailId = detailToEdit.parent_detail_id;
        detailToEdit = state.detailDataCache[detailId];
        if (!detailToEdit) {
            showToast('error', 'Data induk dari rincian ini tidak ditemukan.');
            return;
        }
    }
    ...
    form.dataset.idDetailAsli = detailId; // Simpan ID asli dari header/item tunggal

    let originalChildIds = [];
    if (detailToEdit.children && detailToEdit.children.length > 0) {
        originalChildIds = detailToEdit.children.map(child => child.id_rab_detail);
    }

    form.dataset.originalChildIds = JSON.stringify(originalChildIds);

    let allDetailRowsHTML = '';
    
    // Logika untuk mengisi form: Cek apakah ini header atau bukan
    if (detailToEdit.children && detailToEdit.children.length > 0) {
    ...
    const oldCatatanContainer = form.querySelector('#catatan-revisi-container');
    if(oldCatatanContainer) oldCatatanContainer.remove();

    const catatanRevisiContainer = document.createElement('div');
    catatanRevisiContainer.id = 'catatan-revisi-container';
    form.insertBefore(catatanRevisiContainer, form.querySelector('.modal-form-footer'));

    if (mode === 'revisi') {
        modalTitle.textContent = 'Formulir Pengajuan Revisi';
        updateBtn.textContent = 'Ajukan Revisi';
        catatanRevisiContainer.innerHTML = `
            <div class="form-header-group" style="margin-top: 15px;">
                <label for="input-catatan-revisi">Catatan / Alasan Revisi (Wajib Diisi)</label>
                <textarea id="input-catatan-revisi" placeholder="Jelaskan alasan pengajuan revisi..." rows="3" style="width:100%; padding:8px; border-radius: 4px; border: 1px solid #ccc;"></textarea>
            </div>
        `;
    } else {
